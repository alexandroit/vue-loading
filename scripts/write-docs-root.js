...
function buildCards() {
  return releaseLines.map((line) => {
    ...
    return [
      ...
      '          </div>',
      `          <h2>${line.label}</h2>`,
      `          <p>Maintained Vue wrapper docs for the ${line.runtime} runtime line, keeping the same ReviveJS loading engine with the Vue conventions expected by that generation.</p>`,
...
fs.writeFileSync(
  ...
  [
    ...
    '    <title>@revivejs/vue-loading</title>',
    '    <style>',
    '      :root {',
    '        color-scheme: light;',
    ...
    '    <main>',
    '      <section class="hero">',
    '        <span class="eyebrow">ReviveJS Vue loading wrapper</span>',
    '        <h1>@revivejs/vue-loading</h1>',
    '        <p>Versioned Vue bindings for the ReviveJS loading toolkit. Each maintained line keeps the same loader engine and public ideas while fitting the runtime expectations of Vue 2 and Vue 3.</p>',
    '        <div class="hero-actions">',
    '          <a class="hero-link primary" href="./vue-3/">Open Vue 3 docs</a>',
    '          <a class="hero-link secondary" href="https://www.npmjs.com/package/@revivejs/vue-loading">npm package</a>',
    '        </div>',
    '      </section>',
